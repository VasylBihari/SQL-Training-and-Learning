...
WITH spent_table AS (SELECT 
  d.country_name AS country,
  SUM(amount_spent) AS total_spent
FROM dimension_country d 
LEFT JOIN fact_daily_subscriptions f ON f.country_id=d.country_id
LEFT JOIN fact_marketing_spend s ON d.country_id=s.country_id
WHERE s.campaign_date BETWEEN '2024-01-01' AND '2024-03-31'
GROUP BY d.country_name
  ),
num_subscr_table AS (
  SELECT 
  d.country_name AS country,
  SUM(num_new_subscribers) AS total_num_subscr
FROM dimension_country d 
LEFT JOIN fact_daily_subscriptions f ON f.country_id=d.country_id
LEFT JOIN fact_marketing_spend s ON d.country_id=s.country_id
WHERE f.signup_date BETWEEN '2024-01-01' AND '2024-03-31'
GROUP BY d.country_name
)
SELECT
  s.country,
  CEIL(total_spent/total_num_subscr) AS average_by_country
FROM spent_table s 
LEFT JOIN num_subscr_table n ON s.country=n.country
